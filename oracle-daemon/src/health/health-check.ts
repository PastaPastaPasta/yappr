...
export class HealthChecker {
   ...
   */
  async checkDashCore(): Promise<void> {
    try {
   ...
   */
  updateSyncStatus(
    type: 'proposals' | 'votes' | 'masternodes',
    success: boolean,
    count: number
  ): void {
  ...
  getStatus(): HealthStatus {
    ...
    const config = getConfig();
    const proposalsSyncHealthy =
      (now - this.syncStatus.proposals.timestamp < config.sync.proposalIntervalMs * 2) &&
      this.syncStatus.proposals.success;
    const votesSyncHealthy =
      (now - this.syncStatus.votes.timestamp < config.sync.voteIntervalMs * 2) &&
      this.syncStatus.votes.success;
    const masternodesSyncHealthy =
      (now - this.syncStatus.masternodes.timestamp < config.sync.masternodeIntervalMs * 2) &&
