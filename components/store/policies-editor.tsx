      ...
      {policies.length === 0 ? (
        ...
        <div className="space-y-4">
          {policies.map((policy, index) => (
            <div
              key={index}
              className="p-4 border border-surface-200 dark:border-neutral-750 rounded-lg space-y-3"
            >
              {/* Policy Name */}
              <div className="relative" ref={openSuggestionIndex === index ? suggestionRef : undefined}>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <input
                      ...
                      maxLength={100}
                      className="w-full px-4 py-2 pr-10 border border-surface-200 dark:border-neutral-750 rounded-lg bg-white dark:bg-surface-900 focus:outline-none focus:ring-2 focus:ring-yappr-500"
                    />
                    <button
                      type="button"
                ...
                {openSuggestionIndex === index && (
                  <div className="absolute z-10 mt-1 w-full bg-white dark:bg-surface-900 border border-surface-200 dark:border-neutral-750 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                    {getAvailableSuggestions(index).length > 0 ? (
                      getAvailableSuggestions(index).map((name) => (
                        <button
                          ...
                          className="w-full px-4 py-2 text-left text-sm hover:bg-surface-100 dark:hover:bg-surface-800 first:rounded-t-lg last:rounded-b-lg"
                        >
                          {name}
                        </button>
                      ))
