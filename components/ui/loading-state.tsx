...
export function LoadingState({
    ...
    return (
      <div className={`flex flex-col items-center justify-center p-12 ${className}`}>
        <motion.div
          ...
          className="rounded-full h-8 w-8 border-2 border-yappr-200 border-t-yappr-500 dark:border-zinc-700 dark:border-t-yappr-500 mb-4"
        />
        <p className="text-zinc-500 dark:text-zinc-400 text-sm">{loadingText}</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className={`flex flex-col items-center justify-center p-12 ${className}`}>
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-center mb-5 max-w-md text-sm">
          {error}
        </p>
        {onRetry && (
          <button
            onClick={onRetry}
            className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-yappr text-white rounded-xl hover:brightness-110 transition-all text-sm font-medium"
          >
  ...
  if (isEmpty) {
    return (
      <div className={`flex flex-col items-center justify-center p-12 ${className}`}>
        <div className="w-14 h-14 rounded-2xl bg-zinc-100 dark:bg-zinc-800/50 flex items-center justify-center mb-4">
