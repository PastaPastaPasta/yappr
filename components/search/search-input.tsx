  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className="w-full h-10 pl-10 pr-10 bg-surface-1 dark:bg-surface-2 border border-transparent rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-yappr-500/40 focus:border-yappr-500/50 focus:bg-white dark:focus:bg-surface-3 transition-all"
      />
      {query && (
        <button
          onClick={handleClear}
          className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-surface-2 dark:hover:bg-surface-3 rounded-lg z-10 transition-colors"
        >
      ...
      <AnimatePresence>
        {showDropdown && query.trim().length >= MIN_SEARCH_LENGTH && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-surface-1 rounded-2xl shadow-lg border border-border overflow-hidden z-50"
          >
            {isSearching ? (
              ...
              <div className="max-h-64 overflow-y-auto">
                {results.map((result, index) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 text-left transition-colors ${
                      index === selectedIndex
                        ? 'bg-yappr-50 dark:bg-yappr-950/30'
                  ...
                  >
                    {result.type === 'user' ? (
                      ...
                      <>
                        <div className="h-10 w-10 rounded-full bg-yappr-100 dark:bg-yappr-950/50 flex items-center justify-center flex-shrink-0">
                ...
                <button
                  ...
                  className="w-full p-3 text-sm text-yappr-500 hover:bg-surface-1 dark:hover:bg-surface-2 text-center border-t border-border transition-colors"
                >
