...
export function PostAuthorLine({ author, usernameState, displayName, avatarUrl, hasProfile, profileLoaded }: PostAuthorLineProps) {
  const copy = useCopy()
    ...
    if (usernameState) {
      return (
        <ProfileHoverCard {...hover} username={usernameState}>
          <Link href={`/user?id=${author.id}`} onClick={stop} className="text-gray-500 hover:underline truncate">
            @{usernameState}
          </Link>
        </ProfileHoverCard>
      )
  ...
  return (
    <>
      {usernameState === undefined || (!hasProfile && !profileLoaded) ? (
        ...
        <ProfileHoverCard {...hover} username={usernameState}>
          <Link href={`/user?id=${author.id}`} onClick={stop} className="font-semibold hover:underline truncate">
