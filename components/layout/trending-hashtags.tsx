 ...
 * every page costs one ranked query per session window.
 */
export function TrendingHashtags() {
  ...
  useEffect(() => {
    let cancelled = false
    hashtagService
      .getTrendingHashtags({ timeWindowHours: 168, minPosts: 1, limit: TRENDING_LIMIT, window: 'all' })
      .then((tags) => {
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-950 rounded-2xl overflow-hidden" data-testid="sidebar-trending">
      ...
      {trending === null ? (
        ...
        <p className="px-4 pb-4 text-sm text-gray-500">No trending tags yet</p>
      ) : (
        <ul>
