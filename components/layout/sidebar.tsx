    ...
    <div className="hidden md:flex h-[calc(100vh-40px)] w-[275px] shrink-0 flex-col px-3 sticky top-[40px]">
      <div className="flex-1 space-y-1 py-4 overflow-y-auto scrollbar-hide">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive ? 'font-semibold text-gray-900 dark:text-white bg-surface-1 dark:bg-surface-2' : 'text-gray-600 dark:text-gray-400'
                )}
              >
                <div className="relative">
                  <Icon className={cn('h-6 w-6', isActive && 'text-yappr-500')} />
                  {showBadge && (
                    <span className="absolute -top-1.5 -right-1.5 flex h-[18px] min-w-[18px] items-center justify-center rounded-full bg-gradient-yappr text-[10px] font-bold text-white px-1">
          ...
          {user && isHydrated && (
            <Link
              ...
              className={cn(
                'flex items-center gap-4 px-3 py-3 text-[15px] rounded-xl transition-all duration-200',
                'hover:bg-surface-1 dark:hover:bg-surface-2',
                pathname === '/settings' ? 'font-semibold text-gray-900 dark:text-white bg-surface-1 dark:bg-surface-2' : 'text-gray-600 dark:text-gray-400'
              )}
            >
              <Cog6ToothIcon className={cn('h-6 w-6', pathname === '/settings' && 'text-yappr-500')} />
              <span>Settings</span>
            </Link>
          )}
        </nav>
        ...
        {isHydrated && user ? (
          ...
          <div className="mt-6">
            <div className="w-full h-12 skeleton rounded-full" />
          </div>
        )}
      </div>
        ...
        {user && isHydrated && (
          <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
              <button className="flex items-center gap-3 p-3 rounded-xl hover:bg-surface-1 dark:hover:bg-surface-2 transition-colors w-full">
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.Content
                className="min-w-[220px] bg-white dark:bg-surface-1 rounded-2xl shadow-lg border border-border py-2 z-50"
                sideOffset={8}
              >
                <DropdownMenu.Item
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg hover:bg-surface-1 dark:hover:bg-surface-2 transition-colors"
                      title="Refresh balance"
                    >
                      <ArrowPathIcon className={`h-4 w-4 text-gray-400 ${isRefreshingBalance ? 'animate-spin' : ''}`} />
                    </button>
                  </div>
                </DropdownMenu.Item>
                <DropdownMenu.Separator className="h-px bg-border my-1" />
                <DropdownMenu.Item
                  className="px-4 py-2.5 text-sm hover:bg-surface-1 dark:hover:bg-surface-2 cursor-pointer outline-none flex items-center gap-2.5 text-red-500 rounded-lg mx-1"
                  onClick={logout}
                >
