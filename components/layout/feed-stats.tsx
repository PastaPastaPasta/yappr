    ...
    <div className="space-y-2">
      {[...Array(rows)].map((_, i) => (
        <div key={i} className="flex justify-between">
          ...
          <div className="h-4 w-8 bg-surface-200 dark:bg-surface-800 rounded animate-pulse" />
        </div>
      ))}
    </div>
  )
...
export function FeedStats() {
  ...
  return (
    <div ref={containerRef} className="bg-surface-100 dark:bg-surface-800 rounded-2xl overflow-hidden border border-surface-200 dark:border-neutral-750">
      <h2 className="text-lg font-bold px-4 py-3 flex items-center gap-2 text-gray-900 dark:text-gray-100">
        ...
        {!shouldLoad || globalLoading || !globalStats ? (
          ...
          <>
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-gray-900 dark:text-gray-100">{formatNumber(globalStats.totalPosts)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500 dark:text-gray-400">Active Users</span>
              <span className="font-medium text-gray-900 dark:text-gray-100">{formatNumber(globalStats.activeUsers)}</span>
            </div>
          </>
        )}
        {user && (
          <>
            {!shouldLoad || loading || !stats ? (
              ...
              <>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Your Posts</span>
                  <span className="font-medium text-gray-900 dark:text-gray-100">{formatNumber(stats.posts)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Followers</span>
                  <span className="font-medium text-gray-900 dark:text-gray-100">{formatNumber(stats.followers)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Following</span>
                  <span className="font-medium text-gray-900 dark:text-gray-100">{formatNumber(stats.following)}</span>
                </div>
              </>
            )}
          </>
