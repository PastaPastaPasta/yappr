...
class HashtagService extends BaseDocumentService<PostHashtagDocument> {
  ...
  async createPostHashtag(postId: string, ownerId: string, hashtag: string): Promise<boolean> {
    // On the inline-hashtag topology the postHashtag doctype does not exist:
    // a post's one tag is written with the post and cannot be added afterwards.
  ...
  async getPostCountByHashtag(hashtag: string): Promise<number> {
    try {
      ...
      // No postHashtag count tree — count posts on post.tagAndTime instead.
      if (hashtagsAreInline()) {
  ...
  async getPostIdsByHashtag(hashtag: string): Promise<PostHashtagDocument[]> {
    try {
      // No postHashtag documents to list — tag pages query post.tagAndTime
  ...
  async getTrendingHashtags(options: {
    ...
    minPosts?: number;
    limit?: number;
    /** `'today'` reads the daily-windowed ranking (`beat.byDayHashtagPost`); default all-time. */
    window?: 'all' | 'today';
    ...
    // Where the prefix ranked axes exist (v7), trending is a PROVED prefix
    // ranked page — groupBy at `hashtag` on `like.byHashtagPost {at: hashtag}`
    // (skipIfAbsent, so only tagged likes are in the index). NOTE the metric
    // change: the count is then LIKES ON TAGGED POSTS per tag, not post-count —
    // a ranking of tag engagement rather than tag usage. It rides the
    // TrendingHashtag shape unchanged; consumers that print a unit label gate on
    // `prefixRankingsAvailable()`. Fail-soft: on any error (e.g. a node that
    // cannot serve the prefix form yet) trending degrades to empty rather than
    // silently reporting the other metric.
    ...
    try {
      ...
      // Cache the full result (the postHashtag count is all-time by construction)
