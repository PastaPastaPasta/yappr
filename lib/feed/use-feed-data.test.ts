...
beforeEach(() => {
  ...
  mocks.cached.mockReturnValue([cachedPost]);
  mocks.enrich.mockReturnValue(new Promise<Post[]>(resolve => { complete = resolve; }));
  mocks.load.mockResolvedValue({ posts: [freshPost], hasMore: false, cursor: freshPost.id });
});
