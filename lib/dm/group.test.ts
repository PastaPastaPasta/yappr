...
describe('keyring slots (§4.5, §5.3)', () => {
  ...
  it('lets each remaining member, and only them, unwrap K[b,0]', () => {
    const baseKey = deriveBaseKey(SECRET, 1)
    const keyring = buildKeyring({
      ...
      b: 1,
      baseKey,
      members: [{ id: BOB_ID, publicKey: BOB_PUB }],
    })
    expect(keyring).toHaveLength(8 + 8 * 32)
    expect(hex(keyring.slice(0, 8))).toBe(hex(keyCheck(baseKey)))
  ...
  it('fails on a tampered kc or slot, and on a malformed keyring', () => {
    const baseKey = deriveBaseKey(SECRET, 1)
    const keyring = buildKeyring({ ownerPrivateKey: ALICE_PRIV, ownerId: ALICE_ID, gid: GID, b: 1, baseKey, members: [{ id: BOB_ID, publicKey: BOB_PUB }] })
    const asBob = { gid: GID, ownerId: ALICE_ID, memberId: BOB_ID, b: 1, myPrivateKey: BOB_PRIV, otherPublicKey: ALICE_PUB }
    const badKc = keyring.slice()
    badKc[0] ^= 1
    expect(openKeyringSlot(badKc, asBob)).toBeNull()
    const allSlotsFlipped = keyring.map((b, i) => (i >= 8 && i % 32 === 8 ? b ^ 1 : b))
    expect(openKeyringSlot(allSlotsFlipped, asBob)).toBeNull()
    expect(openKeyringSlot(keyring.slice(0, 8), asBob)).toBeNull()
    expect(openKeyringSlot(keyring.slice(0, 20), asBob)).toBeNull()
  })

  it('rejects more than 99 non-owner members and a short base key', () => {
    const baseKey = deriveBaseKey(SECRET, 3)
    const params = { ownerPrivateKey: ALICE_PRIV, ownerId: ALICE_ID, gid: GID, b: 3, baseKey, members: [{ id: BOB_ID, publicKey: BOB_PUB }] }
    expect(() => buildKeyring({ ...params, members: Array.from({ length: 100 }, () => params.members[0]) })).toThrow('Too many')
    expect(() => buildKeyring({ ...params, baseKey: baseKey.slice(1) })).toThrow('32 bytes')
  ...
  it('fits a 100-member group (99 slots → 128) in one field, with the slot order shuffled', () => {
    ...
    const baseKey = deriveBaseKey(SECRET, 3)
    const params = { ownerPrivateKey: ALICE_PRIV, ownerId: ALICE_ID, gid: GID, b: 3, baseKey, members }
    const keyring = buildKeyring(params)
    expect(keyring).toHaveLength(8 + 128 * 32)
    ...
    // Two builds of the same keyring differ (random filler, random order).
    expect(hex(buildKeyring(params))).not.toBe(hex(keyring))
  })
})
...
describe('roster (§5.4)', () => {
  ...
  it('fails with a different base, a different group handle, or tampering', async () => {
    ...
    expect(await openRoster({ blob, gid: GID, known: { b: 1, r: 0, key: deriveBaseKey(SECRET, 1) }, maxSteps: 5 })).toBeNull()
