...
- `loginKeyResponse`: make the QR handshake doctype indexOnly with a timeRange index carrying `ttl`, so responses self-expire. `ttl` drains only the entries under that index; on a normal doctype the body would persist, so self-deletion needs indexOnly.
